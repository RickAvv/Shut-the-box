...
table_shut = np.zeros((len(all_combinations), 13), dtype=np.object)
table_mean = np.zeros((len(all_combinations), 13), dtype=np.object)
comb_idx = -1
tot_comb = len(all_combinations)

for start in all_combinations:
    comb_idx += 1

    for r in range(1,13):
        a = choose_and_flip_best_prob(start,r)
        b = choose_and_flip_best_mean(start,r)
        table_shut[comb_idx][r] = a[1]
        table_mean[comb_idx][r] = b[1]
        
np.save("table_shut.npy", table_shut)
np.save("table_mean.npy", table_mean)
